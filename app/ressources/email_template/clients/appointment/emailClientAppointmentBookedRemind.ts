...
export const emailClientAppointmentRemind = (data: EmailData) => {
    return `<html lang="fr">
<head>
  <meta charset="UTF-8">
  <title>Rappel de Rendez-vous - ${data?.plateformName ?? process.env.APP_NAME}</title>
</head>
<body style="margin:0; padding:0; font-family:Arial, Helvetica, sans-serif; background-color:#f0f0f0; color:#000;">
  ...
  <div style="max-width:600px; margin:0 auto; background-color:#ffffff; padding:20px;">
    ...
    <div style="padding-bottom:20px; text-align:center;">
      ...
      <div style="font-size:22px; font-weight:bold; margin:10px 0;">Votre rendez-vous commence dans quelques minutes</div>
    </div>
    ...
    <div style="background-color:#f8f9fa; border:2px solid #000; border-radius:8px; padding:20px; margin:20px 0; text-align:center;">
      ...
      <div style="font-size:14px; color:#666; margin-top:5px;">Merci de bien vouloir vous présenter à temps</div>
    </div>

    <!-- Message -->
    <div style="margin:20px 0; font-size:15px; line-height:1.7;">
      Bonjour${data?.fullname ? ` ${data.fullname}` : ""},<br><br>
      Ceci est un rappel que votre rendez-vous pour le service <b>${data?.service}</b> débutera dans quelques minutes.<br>
      Nous vous attendons le <b>${data?.date ? formatDateHelpers(data.date) : ""}</b> dans notre magasin <b>${data?.magasin}</b>.<br>
      <br>
      Merci d'arriver quelques minutes à l'avance pour faciliter votre prise en charge.<br>
      <br>
      Toute l'équipe de <b>${data?.plateformName ?? process.env.APP_NAME}</b> reste à votre disposition pour vous garantir la meilleure expérience possible.
    </div>
      ...
      <table style="width:100%; font-size:15px; border-collapse:collapse;">
        <tbody>
          ...
          <tr>
            ...
            <td style="padding:10px 5px; color: #FBBC05">Rappel : rendez-vous imminent</td>
          </tr>
        </tbody>
      </table>
    </div>
