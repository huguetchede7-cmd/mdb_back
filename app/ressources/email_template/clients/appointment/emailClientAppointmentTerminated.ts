...
export const emailClientAppointmentTerminated = (data: EmailData) => {
    return `<html lang="fr">
<head>
  <meta charset="UTF-8">
  <title>Rendez-vous terminé - ${data?.plateformName ?? process.env.APP_NAME}</title>
</head>
<body style="margin:0; padding:0; font-family:Arial, Helvetica, sans-serif; background-color:#f0f0f0; color:#000;">
  <div style="max-width:600px; margin:0 auto; background-color:#ffffff; padding:20px;">
    ...
    <div style="padding-bottom:20px; text-align:center;">
      ...
      <div style="font-size:22px; font-weight:bold; margin:10px 0; color:#000000;">Votre rendez-vous est terminé avec succès</div>
    </div>

    <!-- Message -->
    <div style="margin:20px 0; font-size:15px; line-height:1.7;">
      Bonjour,<br><br>
      Votre rendez-vous du <b>${data?.date ? formatDateHelpers(data.date) : ""}</b> pour le service <b>${data?.service}</b> s'est déroulé avec succès.<br><br>
      Toute l'équipe <b>OHO Les Lunettes</b> vous remercie pour votre confiance et espère que l'expérience a été à la hauteur de vos attentes.  
      <br><br>
      Nous serons heureux de vous revoir pour vos prochains besoins en confort visuel.
    </div>
      ...
      <table style="width:100%; font-size:15px; border-collapse:collapse;">
        <tbody>
          ...
          <tr>
            ...
            <td style="padding:10px 0; color:#27ae60;"><b>Terminé avec succès</b></td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div style="background:#000; color:#fff; padding:25px 15px; font-size:13px; text-align:center; line-height:1.8;">
      Merci pour votre confiance et à très bientôt chez <b>${data?.plateformName ?? process.env.APP_NAME}</b>.<br><br>
