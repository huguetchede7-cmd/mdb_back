...
export const emailWelcomeTemplateWithEmailValidation = (data: EmailData) => {
    const activationSection = data?.activationLink ? `
         <p style="color: #666666; margin: 20px 0; font-size: 16px; line-height: 1.5;">Pour activer votre compte et profiter de nos services, veuillez confirmer votre adresse email en cliquant sur le lien ci-dessous :</p>
        <div style="margin: 30px 0; text-align: center;">
            <a href="${data?.activationLink}" style="display: inline-block; word-break: break-all; color: #007BFF; text-decoration: underline; font-size: 15px;">${data?.activationLink}</a>
        </div>
    ` : ''
                            ...
                            <tr>
                                <td style="padding: 20px;">
                                   ...
                                   ${activationSection}
                                    ...
                                    <p style="color: #888888; margin: 20px 0 0; font-size: 14px; font-style: italic;">Si vous n'avez pas créé de compte sur ${data?.plateformName ?? process.env.APP_NAME}, veuillez ignorer ce message.</p>
                                </td>
                            </tr>
                            <tr>
                                <td style="padding: 20px; text-align: center; border-top: 1px solid #eeeeee;">
                                    <p style="color: #888888; margin: 0; font-size: 14px;">&copy; ${data?.year} ${data?.plateformName ?? process.env.APP_NAME}. Tous droits réservés.</p>
                                </td>
                            </tr>
                        </table>
                    </td>
